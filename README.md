...
- **Session name in the window title** (on by default, see Options): the
  session name is written into the window title (`titlePreface`, shown in
  square brackets) when you track, rename, or restore a session, so windows
  stay visibly labeled — the same mechanism Window Titler uses. Renaming in
  the sidebar updates the title bar immediately. The extension keeps re-owning
  the title — on every wake, after tab activity, and on a periodic tick — so a
  preface lost while the background was idle, or overwritten by another titling
  extension, is restored automatically. Caveat: Window Titler's own stored
  names cannot be updated from outside, so if you also name the same window
  there, the two extensions compete; because this extension reasserts
  continuously it generally wins on tracked windows. Best used as a
  replacement for Window Titler on tracked windows.
